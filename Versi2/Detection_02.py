                        ...
                        for result in license_plate_results:
                            for lp_box in result.boxes:
                                ...
                                if lp_conf > 0.1:
                                    ...
                                    filename = rtsp_url.replace('DataVideo/','')
                                    filename = filename.replace('.mp4', '')

                                    cv2.imwrite(filename=f"Plate/{filename}-{frame_count}",
                                                img=license_crop)
                                    # # OCR
                                    # plat_number, plat_conf = op.f_pytesseract_process(license_crop)
                                    # if plat_number:
