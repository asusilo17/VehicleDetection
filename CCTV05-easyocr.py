...
rtsp_url = 'DataVideo/A4574WAD.mp4'
                ...
                if class_id in [2, 3, 5, 7] and confidence > 0.10:
                    ...
                    for result in license_plate_results:
                        for lp_box in result.boxes:
                            ...
                            if lp_conf > 0.1:
                                ip.f_image_box(frame, lx1, lx2, ly1, ly2, lp_conf)
                                ...
                                # # ip.f_image_box(frame, abs_lx1, abs_lx2, abs_ly1, abs_ly2, lp_conf)
                                ...
                                # #####################################################################
                                # # plat_number, plat_conf = op.f_easyocr_process(license_crop)
                                # plat_number, plat_conf = op.f_ocr_process(license_crop)

                                # if plat_number:
                                #     frame = ip.f_image_box_license_2(frame, abs_lx1, abs_lx2, abs_ly1, abs_ly2, plat_number)

                                    # sqlp.f_sql_savePlateIdentification(p_file_name=rtsp_url,
                                    #                                 p_license_number=plat_number,
                                    #                                 p_confidence=lp_conf,
                                    #                                 p_metode=ocr_metode)
                                    
                                    # dataWanprestasi = sqlp.f_sql_checking_wanprestasi_data(plat_number, iot_location_id)
